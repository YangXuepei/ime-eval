...
# Note that this python file should be totally blinded with any practical
# details, for generic purpose.
...
def simulated_annealing(domain, costfunc):
    ...
    while annealing_temperature > 0.1:
        best_config = compare_with_one_neighbor(
            best_config, domain, costfunc, annealing_temperature)
...
def compare_with_one_neighbor(config, domain, costfunc, annealing_temperature):
    ...
    tolerance_probability = pow(
        math.e, -(neighbor_cost - min_cost) / annealing_temperature)
    # Note that we allow shifting to neighbor even if the cost are equal.
    if neighbor_cost <= min_cost or random.random() < tolerance_probability:
