...
writer = csv.writer(new_table, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

for i in train_character_frequency:
    for j in all_pinyin:
        if i == j:
            print(i.encode('utf-8'), train_character_frequency[i], all_pinyin[i])
            writer.writerow([i.encode('utf-8'), train_character_frequency[i], all_pinyin[i]])